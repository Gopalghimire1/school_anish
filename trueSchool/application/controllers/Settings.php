...
class Settings extends Admin_Controller
{
    ...
    public function signature(){
        if($_FILES['signature_photo']){
            // var_dump($_FILES);die;
            if(is_superadmin_loggedin()){
                $upload_location = $_SERVER['DOCUMENT_ROOT']. '/uploads/signature/superadmin/';
                $save['user_type'] = 'superadmin';
            }
            if(is_accountant_loggedin()){
                $upload_location = $_SERVER['DOCUMENT_ROOT']. '/uploads/signature/accountant/';
                $save['user_type'] = 'accountant';
            }
            if(is_librarian_loggedin()){
                $upload_location = $_SERVER['DOCUMENT_ROOT']. '/uploads/signature/librarian/';
                $save['user_type'] = 'librarian';
            }
            $ext = array_pop(explode('.',$_FILES['signature_photo']['name']));
            $file_name = time().rand(1111,9999).'.'.$ext;
            $save['photo'] = $file_name;
            $save['user_id'] = get_loggedin_user_id();
            $result = $this->db->get_where('signature',array('user_id' => $save['user_id']))->result_array();
            if($result){
                if($this->db->update('signature',$save,array('user_id' => $save['user_id']))){
                    move_uploaded_file($_FILES['signature_photo']['tmp_name'],$upload_location.$file_name);
                }
            } else {
                if($this->db->insert('signature',$save)){
                    move_uploaded_file($_FILES['signature_photo']['tmp_name'],$upload_location.$file_name);
                }
            }
            set_alert('success', translate('information_has_been_updated_successfully'));
            redirect('settings/signature');
        }
        $result = $this->db->get_where('signature',array('user_id' =>get_loggedin_user_id()))->result_array();
        // var_dump($result);die;
        if($result){
            $this->data['photo'] = '/uploads/signature/'.$result[0]['user_type'].'/'.$result[0]['photo'];
        }
        $this->data['title'] = translate('signature');
        $this->data['sub_page'] = 'settings/signature';
        $this->data['main_menu'] = 'settings';
        $this->data['headerelements'] = array(
            'css' => array(
                'vendor/dropify/css/dropify.min.css',
