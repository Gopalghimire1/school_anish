	...
	<div class="col-md-5">
		<section class="panel">
			<header class="panel-heading">
				<h4 class="panel-title"><i class="far fa-edit"></i> <?=translate('privilege_assign')?></h4>
			</header>
			<?php echo form_open($this->uri->uri_string(), array('class' => 'frm-submit'));?>
				<div class="panel-body">
					...
					<div class="form-group mb-md">
						...
						<select id="user" name="user" class="form-control" data-plugin-selectTwo>
						    <option value="" selected disabled>Select</option>
						    <?php 
						    if($teacher_privilege){ 
						        foreach($teacher_privilege as $data){?>
					...
					<div class="row">
						<div class="col-md-12">
							<button class="btn btn-default pull-right" type="submit" name="save" value="1">
								<i class="fas fa-plus-circle"></i> <?=translate('Assign')?>
							</button>
						</div>	
					</div>
				</div>
...
<?php if (get_permission('exam_term', 'is_view')): ?>
	<div class="col-md-<?php if (get_permission('exam_term', 'is_add')){ echo "7"; }else{ echo "12"; } ?>">
		<section class="panel">
			<header class="panel-heading">
				<h4 class="panel-title"><i class="fas fa-list-ul"></i>  <?=translate('privilege_assign') . " " . translate('list')?></h4>
			</header>
			<div class="panel-body">
					...
					<table class="table table-bordered table-hover table-condensed mb-none">
						...
						<tbody>
							...
							<tr>
								...
								<td>
								<?php if (get_permission('exam_term', 'is_delete')): ?>
									<!-- delete link -->
									<?php echo btn_delete('exam/privilege_delete/' . $row['id']);?>
								<?php endif; ?>
								</td>
							</tr>
							<?php
