...
class Student_model extends MY_Model
{
    ...
    public function save($data = array(), $getBranch = array())
    {
        ...
        if (!isset($data['student_id']) && empty($data['student_id'])) {
            ...
            $already_exists = $this->db->get('login_credential')->result_array();
            $this->db->where('user_id', $data['student_id']);
            $this->db->where('role', 7);
            if($already_exists){
                $this->db->update('login_credential', array('username' => $data['username'],'password' => $password));
            }
            else{
                $insert_credential = array(
                    'username' => $username,    
                    'password' => $password,
                ...
                $this->db->insert('login_credential',$insert_credential);
            }
        }
    }
    ...
    public function getStudentList($classID = '', $sectionID = '', $branchID = '', $deactivate = false)
    {
        ...
        $this->db->where('e.session_id', $session);
