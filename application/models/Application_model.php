...
class Application_model extends CI_Model
{
    ...
    public function getUserData($roleID, $userID = '' ){
        if ($roleID == 6) {
            $sql = "SELECT * FROM parent WHERE id = " . $this->db->escape($userID);
            ...
            $sql = "SELECT * FROM staff WHERE id = " . $this->db->escape($userID);
            return $this->db->query($sql)->row_array();
        }
    ...
    {
        $sql = "SELECT e.*, s.photo, CONCAT(s.first_name, ' ', s.last_name) as fullname, s.register_no, s.parent_id, s.email, s.mobileno, s.blood_group, s.birthday, s.admission_date, l.active, c.name as class_name, se.name as section_name, sc.name as category FROM enroll as e INNER JOIN student as s ON e.student_id = s.id LEFT JOIN login_credential as l ON l.user_id = s.id and l.role = 7 LEFT JOIN class as c ON e.class_id = c.id LEFT JOIN section as se ON e.section_id=se.id LEFT JOIN student_category as sc ON sc.id=s.category_id WHERE e.class_id = " . $this->db->escape($classID) . " AND e.branch_id = " . $this->db->escape($branchID) . " AND e.session_id = " . $this->db->escape(get_session_id());
        if ($sectionID != 'all') {
