        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if($this->session->userdata('date_type') === 'bs'){
                    $first_date = explode('-',nepaliToEnglish($daterange[0]));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_eng_date($first_date[0],$first_date[1],$first_date[2]);
                    $start_date = $date['y'].'-'.$date['m'].'-'.$date['d'];
                    $second_date = explode('-',nepaliToEnglish($daterange[1]));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_eng_date($second_date[0],$second_date[1],$second_date[2]);
    ...
    public function leave_check($type_id)
    {
        if (!empty($type_id)) {
            if($this->session->userdata('date_type') === 'bs'){
                ...
                $nep_date1 = explode('-',nepaliToEnglish($daterange[0]));
                $dateConvert = new DateConvert();
                $start_date = $dateConvert->get_eng_date($nep_date1[0],$nep_date1[1],$nep_date1[2]);
                $start = $start_date['y'].'/'.$start_date['m'].'/'.$start_date['d'];
                $nep_date2 = explode('-',nepaliToEnglish($daterange[1]));
                $end_date = $dateConvert->get_eng_date($nep_date2[0],$nep_date2[1],$nep_date2[2]);
                $end = $end_date['y'].'/'.$end_date['m'].'/'.$end_date['d'];
            }
            else{
                $daterange = explode(' - ', $this->input->post('daterange'));   
