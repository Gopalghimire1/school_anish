...
class Employee extends Admin_Controller
{
    ...
    protected function employee_validation()
    {
        ...
        $this->form_validation->set_rules('mobileno[]', translate('mobileno'), 'trim|required');
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if($this->session->userdata('date_type') === 'bs'){
                    $joining_date = explode('-',nepaliToEnglish($post['joining_date']));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_eng_date($joining_date[0],$joining_date[1],$joining_date[2]);
                    $post['joining_date'] = $date['y'].'/'.$date['m'].'/'.$date['d'];
                ...
                $post['mobileno'] = json_encode(implode(',',$post['mobileno']));
