...
class Exam extends Admin_Controller
{
    ...
    {
        if (!get_permission('exam', 'is_view') && !$this->get_exam_privilege()) {
            access_denied();
        }
    ...
    {
        if (!get_permission('exam', 'is_edit') && !$this->get_exam_privilege()) {
            access_denied();
        }
    ...
    {
        if (!get_permission('exam', 'is_delete') && !$this->get_exam_privilege()) {
            access_denied();
        }
        if (!is_superadmin_loggedin()) {
    ...
    {
        if (isset($_POST['save'])) {
            if (!get_permission('exam_term', 'is_add') && !$this->get_exam_privilege()) {
                access_denied();
            }
    ...
    {
        if ($_POST) {
            if (!get_permission('exam_term', 'is_edit') && !$this->get_exam_privilege()) {
                ajax_access_denied();
            }
            $this->term_validation();
            if ($this->form_validation->run() !== false) {
    ...
    {
        if (!get_permission('exam_term', 'is_delete') && !$this->get_exam_privilege()) {
            access_denied();
        }
        if (!is_superadmin_loggedin()) {
            $this->db->where('branch_id', get_loggedin_branch_id());
    ...
    {
        if (isset($_POST['save'])) {
            if (!get_permission('mark_distribution', 'is_add') && !get_exam_privilege()) {
                access_denied();
            }
            if (is_superadmin_loggedin()) {
                $this->form_validation->set_rules('branch_id', translate('branch'), 'required');
    ...
    {
        if ($_POST) {
            if (!get_permission('mark_distribution', 'is_edit') && !$this->get_exam_privilege()) {
                ajax_access_denied();
            }
            if (is_superadmin_loggedin()) {
                $this->form_validation->set_rules('branch_id', translate('branch'), 'required');
    ...
    {
        if (!get_permission('mark_distribution', 'is_delete') && !$this->get_exam_privilege()) {
            access_denied();
        }
        if (!is_superadmin_loggedin()) {
            $this->db->where('branch_id', get_loggedin_branch_id());
    ...
    {
        if (isset($_POST['save'])) {
            if (!get_permission('exam_hall', 'is_add') && !$this->get_exam_privilege()) {
                access_denied();
            }
    ...
    {
        if ($_POST) {
            if (!get_permission('exam_hall', 'is_edit') && !$this->get_exam_privilege()) {
                ajax_access_denied();
            }
            $this->hall_validation();
            if ($this->form_validation->run() !== false) {
    ...
    {
        if (!get_permission('exam_hall', 'is_delete') && !$this->get_exam_privilege()) {
            access_denied();
        }
        if (!is_superadmin_loggedin()) {
            $this->db->where('branch_id', get_loggedin_branch_id());
    ...
    {
        // echo $this->get_exam_privilege(); die;
        if (!get_permission('exam_mark', 'is_add') && !$this->get_exam_privilege()) {
            access_denied();
        }
    ...
    {
        if (!get_permission('exam_grade', 'is_view') && !$this->get_exam_privilege()) {
            access_denied();
        }
    ...
    {
        if (!get_permission('exam_grade', 'is_edit') && !$this->get_exam_privilege()) {
            ajax_access_denied();
        }
    ...
    {
        if (get_permission('exam_grade', 'is_delete') && !$this->get_exam_privilege()) {
            if (!is_superadmin_loggedin()) {
                $this->db->where('branch_id', get_loggedin_branch_id());
            }
            $this->db->where('id', $id);
    ...
    {
        if (!get_permission('report_card', 'is_view') && !$this->get_exam_privilege()) {
            access_denied();
        }
        $branchID = $this->application_model->get_branch_id();
        if ($_POST) {
        ...
        $this->data['exam_privilege'] = $this->get_exam_privilege();
        $this->data['branch_id'] = $branchID;
    ...
    {
        if (!get_permission('tabulation_sheet', 'is_view') && !$this->get_exam_privilege()) {
            access_denied();
        }
        $this->data['branch_id'] = $this->application_model->get_branch_id();
    ...
    public function teacher_privilege(){
        if (!get_permission('exam_timetable', 'is_view')) {
            access_denied();
        }
        // var_dump($_SESSION);die;
        if(is_superadmin_loggedin()){
            $this->data['privileged'] = $this->db->query('SELECT branch.name as branch_name, staff.name, teacher_exam_privilege.id, teacher_exam_privilege.branch_id from teacher_exam_privilege inner join staff on staff.id = teacher_exam_privilege.teacher_id inner join branch on branch.id = teacher_exam_privilege.branch_id')->result_array();
        } else {
            $this->data['privileged'] = $this->db->query('SELECT branch.name as branch_name, staff.name, teacher_exam_privilege.id, teacher_exam_privilege.branch_id from teacher_exam_privilege inner join staff on staff.id = teacher_exam_privilege.teacher_id inner join branch on branch.id = teacher_exam_privilege.branch_id where teacher_exam_privilege.branch_id = "'.$this->application_model->get_branch_id().'"')->result_array();
        }
        $this->data['teacher_privilege'] = $this->db->query('SELECT staff.name, staff.id from staff left join login_credential on login_credential.user_id = staff.id where login_credential.role = 3')->result(); 
        
        if($_POST){
                if($this->input->post('branch_id')){
                    $save['branch_id'] = $this->input->post('user');
                }
                else{
                    $save['branch_id'] = $this->application_model->get_branch_id();
                }
                
                $save['teacher_id'] = $this->input->post('user');
                $save['created_at'] = date('Y-m-d h:i:s');
                if($this->db->get_where('teacher_exam_privilege',array('branch_id'=>$save['branch_id'],'teacher_id'=>$save['teacher_id']))->result()){
                        set_alert('success', translate('teacher_already_exists'));
                        $url = base_url('exam/teacher_privilege');
                        $array = array('status' => 'success', 'url' => $url);
                }else{
                    if($this->db->insert('teacher_exam_privilege',$save)){
                        set_alert('success', translate('information_has_been_saved_successfully'));
                        $url = base_url('exam/teacher_privilege');
                        $array = array('status' => 'success', 'url' => $url);
                    } else {
        ...
        $this->data['branch_id'] = $this->application_model->get_branch_id();
        $this->data['title'] = translate('teacher_privilege');
        $this->data['sub_page'] = 'exam/teacher_privilege';
        $this->data['main_menu'] = 'teacher_privilege';
        $this->load->view('layout/index', $this->data);
    }
    public function privilege_delete(){
        if (get_permission('exam_grade', 'is_delete')) {
            ...
            $this->db->where('id', $id);
            $this->db->delete('teacher_exam_privilege');
        }
    }
