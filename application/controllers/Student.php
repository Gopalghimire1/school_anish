...
class Student extends Admin_Controller
{
    ...
    protected function student_validation()
    {
        ...
        $this->form_validation->set_rules('roll', translate('roll_number'), 'trim|numeric');
    ...
    public function add()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $post['mobileno'] = json_encode($post['mobileno']);
                if(!isset($_POST['guardian_chk'])) {
                    $post['grd_mobileno'] = json_encode($post['grd_mobileno']);
                }
                //convert nepali date to english if nepali
                if($this->session->userdata('date_type') === 'bs'){
                    $admission_date = explode('-',nepaliToEnglish($post['admission_date']));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_eng_date($admission_date[0],$admission_date[1],$admission_date[2]);
                    $post['admission_date'] = $date['y'].'-'.$date['m'].'-'.$date['d'];
                    if(isset($post['birthday']) && $post['birthday'] !== ''){
                        $birthday = explode('-',nepaliToEnglish($post['birthday']));
                        $dateConvert = new DateConvert();
                        $date = $dateConvert->get_eng_date($birthday[0],$birthday[1],$birthday[2]);
                        $post['birthday'] = $date['y'].'-'.$date['m'].'-'.$date['d'];
                    }
                }
                // var_dump($post);die;
        ...
        if (isset($_POST['search'])) {
            ...
            if($this->session->userdata('date_type') === 'bs'){
                if($this->data['students']){
                    $admission_date = explode('-',nepaliToEnglish($this->data['students'][0]['admission_date']));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_nepali_date($admission_date[0],$admission_date[1],$admission_date[2]);
                    $this->data['students'][0]['admission_date'] = $date['y'].'/'.$date['m'].'/'.$date['d'];
                    $birthday = explode('-',nepaliToEnglish($this->data['students'][0]['birthday']));
                    $date = $dateConvert->get_nepali_date($birthday[0],$birthday[1],$birthday[2]);
                    $this->data['students'][0]['birthday'] = $date['y'].'/'.$date['m'].'/'.$date['d'];
                }
    ...
    public function quickDetails()
    {
        ...
        $data['mobileno'] = empty($row->mobileno) ? "N/A" : implode(',',json_decode($row->mobileno));
        $data['state'] = empty($row->state) ? "N/A" : $row->state;
        $data['address'] = empty($row->current_address) ? "N/A" : $row->current_address;
            //convert nepali date to english if nepali
            if($this->session->userdata('date_type') === 'bs'){
                if($data['admission_date'] !== 'N/A'){
                    $data['admission_date'] = date('Y-m-d',strtotime($data['admission_date']));
                    $admission_date = explode('-',nepaliToEnglish($data['admission_date']));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_nepali_date($admission_date[0],$admission_date[1],$admission_date[2]);
                    $data['admission_date'] = englishToNepali($date['y']).'/'.englishToNepali($date['m']).'/'.englishToNepali($date['d']);
                }
                if($data['birthday'] !== 'N/A'){
                    $data['birthday'] = date('Y-m-d',strtotime($data['birthday']));
                    $birthday = explode('-',nepaliToEnglish($data['birthday']));
                    $dateConvert = new DateConvert();
                    $date = $dateConvert->get_nepali_date($birthday[0],$birthday[1],$birthday[2]);
                    $data['birthday'] = englishToNepali($date['y']).'/'.englishToNepali($date['m']).'/'.englishToNepali($date['d']);
                }
            }
    ...
    public function getNewRoll(){
        $class_id = $this->input->post('class_id');
        $section_id = $this->input->post('section_id');
        if($class_id && $section_id){
            $result = $this->db->query('SELECT roll from enroll WHERE class_id = '.$class_id.' AND section_id = ' .$section_id .' order by roll desc')->result_array();
            $roll = $result[0]['roll']; 
            echo json_encode($roll + 1);die;
        }
        echo 0;die;
    }
    ...
    public function shuffle(){
        ...
        if($_POST){
            $class_id = $this->input->post('class_id');
            $section_id = $this->input->post('section_id');
            $shuffle_type = $this->input->post('shuffle_type');
            if($shuffle_type === 'alphabet'){
                $results = $this->db->query('SELECT enroll.id, enroll.roll, student.first_name, student.last_name from enroll LEFT JOIN student ON student.id = enroll.student_id WHERE class_id = '.$class_id.' AND section_id = ' .$section_id .' order by student.first_name asc')->result_array(); 
            }
            else{
                $results = $this->db->query('SELECT enroll.id, enroll.roll, student.first_name, student.last_name from enroll LEFT JOIN student ON student.id = enroll.student_id WHERE class_id = '.$class_id.' AND section_id = ' .$section_id .' order by student.admission_date asc')->result_array();
            }
            $k = 1;
            foreach($results as $result){
                $this->db->update('enroll',array('roll'=>$k),array('id' => $result['id']));
                $k++;
            }
            if($shuffle_type === 'alphabet'){
                $this->data['students'] = $this->db->query('SELECT enroll.id, enroll.roll, student.first_name, student.last_name from enroll LEFT JOIN student ON student.id = enroll.student_id WHERE class_id = '.$class_id.' AND section_id = ' .$section_id .' order by student.first_name asc')->result_array();    
            }
            else{
                $this->data['students'] = $this->db->query('SELECT enroll.id, enroll.roll, student.first_name, student.last_name from enroll LEFT JOIN student ON student.id = enroll.student_id WHERE class_id = '.$class_id.' AND section_id = ' .$section_id .' order by student.admission_date asc')->result_array();
            }
        }
        
        $this->data['branch_id'] = $this->application_model->get_branch_id();
        $this->data['title'] = translate('shuffle_roll');
        $this->data['sub_page'] = 'student/shuffle';
        $this->data['main_menu'] = 'admission';
        $this->load->view('layout/index', $this->data);
    }
    ...
    public function getRegisterId(){
        ...
        if($branch_id){
            $branch_prefix = $this->db->query('SELECT branch_code FROM branch WHERE id="'.$branch_id.'"')->row_array();
            $result = $this->db->query('SELECT student.id AS id FROM student INNER JOIN student_category ON student.category_id = student_category.id WHERE student_category.branch_id="'.$branch_id.'" order by id desc')->result_array();
            (int)$id = $result[0]["id"];
            // var_dump($id); die;
            if (!empty($id)) {
            ...
            $prefix = $branch_prefix['branch_code'];
            echo json_encode($prefix . $maxNum); die;
        } else {
